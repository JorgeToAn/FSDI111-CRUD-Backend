...
def create(raw_json):
    ...
    """
    conn = get_db()
    conn.execute(statement, (raw_json["name"], raw_json["summary"], raw_json["description"]))
    conn.commit()
    conn.close()
...
def update(raw_json, pk):
    ...
    """
    conn = get_db()
    conn.execute(statement, (raw_json["name"], raw_json["summary"], raw_json["description"], pk))
    conn.commit()
    conn.close()
